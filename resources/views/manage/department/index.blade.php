					...
					<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        ...
                        <div class="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                            <div class="flex flex-col md:flex-row items-stretch md:items-center md:space-x-3 space-y-3 md:space-y-0 justify-between mx-4 py-4 dark:border-gray-700 ">
                                <a href="{{ route('managedepartment.create') }}" class="flex items-center justify-center text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-green-600 dark:hover:bg-green-700 focus:outline-none dark:focus:ring-green-800">
                            ...
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td class="px-6 py-4">
                                            <x-input-label>{{ $departments->deptname }}</x-input-label>
                                        </td>
                                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            <div class="flex items-center">
                                                @if($departments->status == 'Active')
                                                    <div class="h-4 w-4 rounded-full inline-block mr-2 bg-green-700"></div>
                                                @elseif($departments->status == 'Inactive')
                                                    <div class="h-4 w-4 rounded-full inline-block mr-2 bg-red-700"></div>
                                                @endif
                                                <x-input-label for="status" :value="$departments->status"/>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            <div class="flex items-center space-x-2">
                                                <a href="{{ route('managedepartment.edit',$departments->deptid) }}" class="py-2 px-3 flex items-center text-sm font-medium text-center text-white bg-primary-700 rounded-lg hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                                ...
                                                <a href="{{ route('managedepartment.show',$departments->deptid) }}" class="py-2 px-3 flex items-center text-sm font-medium text-center text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                                                ...
                                                <form action="{{ route('managedepartment.destroy',$departments->deptid) }}" method="POST" >
                                                    @csrf
                                                    @method('DELETE')
                                                    @php
                                                    $txtbutton = '';
                        ...
                        <div class="mt-4">
                        {{ $department->appends(request()->query())->links() }}
                        </div>
                    </div>
                </div>
            </div>
